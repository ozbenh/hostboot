 ...
 * MBOX IO accessors
 */
errlHndl_t PnorDD::mboxOut(uint64_t i_addr, uint8_t i_byte)
{
...
errlHndl_t PnorDD::mboxIn(uint64_t i_addr, uint8_t &o_byte)
{
									...
									uint32_t &o_lpcAddr, size_t &o_chunkLen)
{
	...
	do {
		...
		mboxMessage winMsg(i_cmd);
		winMsg.put16(0, l_pos >> iv_blockShift);
		l_err = doMessage(winMsg);
		if (l_err)
			break;
