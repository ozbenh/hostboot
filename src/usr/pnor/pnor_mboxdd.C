									...
									size_t &o_chunkLen)
{
	...
	do {
		...
		/* A read request can be services by a write window */
		if (iv_curWindowOpen &&
		...
		if (iv_protocolVersion == 1) {
			...
			l_reqSize = (((i_reqAddr + i_reqSize) + l_blockMask) & ~l_blockMask) - l_pos;
		}
		...
		astMbox::mboxMessage winMsg(i_isWrite ? astMbox::MBOX_C_CREATE_WRITE_WINDOW :
                                ...
                                const void* i_data )
{
	 ...
	 * and instead had a single flush call over a series of writes. At the
	 * moment I see us erasing & re-writing the same pages at least 3 times
